...
var LEFT_DUDE = Direction.RIGHT;
var RIGHT_DUDE = Direction.LEFT;

var power = {};
power[LEFT_DUDE] = 150;
power[RIGHT_DUDE] = 150;
...
function updatePower() {
	if (power[RIGHT_DUDE] < 150) power[RIGHT_DUDE] += POWER_SPEED*(TIMESLICE/1000);
	if (power[LEFT_DUDE] < 150) power[LEFT_DUDE] += POWER_SPEED*(TIMESLICE/1000);

	$("#left_power").css("width", power[LEFT_DUDE]);
	$("#right_power").css("width", power[RIGHT_DUDE]);
	
//	console.log("LEFT: " + power[LEFT_DUDE] + " right: " + power[RIGHT_DUDE]);
}
/**
...
function fire(thing, dir) {
	return function() {
		var dude = (dir == Direction.RIGHT)? LEFT_DUDE:RIGHT_DUDE;
		if (power[dude] < thing.cost) return;
		power[dude] -= thing.cost;
...
function initWindow() {
	...
	// position the dudes exactly so we know where the hands are
	$("#left_dude").css({
		left: -10,
		top: HEIGHT-450
	}); 
